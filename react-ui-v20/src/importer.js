...
async function importPlaylist(profile, onProgress) {
  ...
  try {
    ...
    if (profile.source === 'xtream') {
      ...
      categoryRows = categoryRows.concat(categoryRowsFor(importId, 'live', liveCategories), categoryRowsFor(importId, 'vod', vodCategories), categoryRowsFor(importId, 'series', seriesCategories));
      ...
      await writeCategories(importId, categoryRows, onProgress);
      ...
      categoryRows = names.map(function (name, index) { return { importId: importId, kind: 'live', name: name, sort: index }; });
      await writeCategories(importId, categoryRows, onProgress);
    }
