...
Le CLI webOS `ares-package`/`ares-install` n’était pas présent dans l’environnement. Le fichier `.ipk` de release a donc été assemblé comme archive Debian webOS à partir du même dossier device, puis inspecté avec `ar` et `tar` : il contient `debian-binary`, `control.tar.gz`, `data.tar.gz`, `appinfo.json`, `index.html`, les bundles Enact, les ressources Sandstone/iLib et les icônes locales.
