...
// Même base IndexedDB que l'application webOS réelle. Cette version React ne
// crée pas de catalogue de démonstration : elle lit uniquement les profils,
// imports, catalogues, EPG et favoris déjà persistés par IPTVDatabase.
...
export async function readProfileData(profile) {
  ...
  const [channels, movies, series, favorites] = await Promise.all([
    iptvDb.channels.where('importId').equals(importId).toArray(),
    iptvDb.vod.where('importId').equals(importId).toArray(),
    iptvDb.series.where('importId').equals(importId).toArray(),
  ...
  return { profile, channels: sortCatalog(channels), movies: sortCatalog(movies), series: sortCatalog(series), favorites, epg, episodes };
}
